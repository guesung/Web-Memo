...
# per_page=20 한 번이면 충분하므로 --paginate와 head를 쓰지 않습니다. pipefail 아래에서 head가
# 파이프를 먼저 닫으면 gh가 SIGPIPE로 죽어 항상 조회 실패로 처리되기 때문입니다.
runs=$(gh api \
  "repos/${GITHUB_REPOSITORY}/actions/workflows/${WORKFLOW_FILE}/runs?branch=${BRANCH}&event=push&status=success&per_page=20" \
  --jq '.workflow_runs[].head_sha' 2>/dev/null) || runs=""
