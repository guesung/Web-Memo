 ...
 *
 * 사용:
 *   node .github/scripts/env/check-env-manifest.mjs          # 검사. 어긋나면 exit 1
 *   node .github/scripts/env/check-env-manifest.mjs --write  # docs/environment-variables.md 의 생성 목록을 갱신
 */
...
const main = async () => {
	...
	if (renderDocs(docsText, entries) !== docsText) {
		errors.push(
			`${DOCS_PATH} 의 생성 목록이 매니페스트와 다릅니다. node .github/scripts/env/check-env-manifest.mjs --write 로 갱신하세요`,
		);
	}
