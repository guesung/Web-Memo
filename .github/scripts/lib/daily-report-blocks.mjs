 ...
 * 같은 기준으로 잰 수치로 읽힙니다.
 */
const buildContextBlock = ({ trafficTotals, unreleasedEvents, runUrl }) => {
	...
	if (unreleasedEvents.length > 0) {
		lines.push(
			`미출시 ${unreleasedEvents.length}종: ${unreleasedEvents.join(", ")}`,
		);
	}
...
export const buildDailyReportPayload = ({ report, runUrl }) => {
	const { targetDate, events, outliers, unreleasedEvents, trafficTotals } =
		report;
	...
	return {
		...
		blocks: [
			...
			buildContextBlock({ trafficTotals, unreleasedEvents, runUrl }),
		],
	};
};
