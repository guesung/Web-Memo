import { useFavorites, useFavoriteRemove } from "@/lib/hooks/useFavorites";
    ...
    >
      <View className="w-14 h-14 rounded-[14px] bg-input justify-center items-center overflow-hidden">
        {item.favIconUrl && !imgError ? (
          ...
          <View className="justify-center items-center bg-[#e0e0e0]" style={{ width: 36, height: 36, borderRadius: 4 }}>
            <Text className="text-base font-bold text-gray-500">
...
function FavoritesSection({ onPress }: { onPress: (url: string) => void }) {
  ...
  const handleLongPress = (url: string, title: string) => {
    Alert.alert("즐겨찾기 삭제", `"${title}" 을(를) 즐겨찾기에서 삭제하시겠습니까?`, [
      { text: "취소", style: "cancel" },
      { text: "삭제", style: "destructive", onPress: () => removeFavorite.mutate(url) },
    ]);
  };
      ...
      <View>
        {rows.map((row, rowIndex) => (
          <View key={`fav-row-${rowIndex}`} className="flex-row mb-4">
            {row.map((fav) => (
              <FavoriteGridItem key={fav.id} item={fav} onPress={onPress} onLongPress={handleLongPress} />
            ))}
            {row.length < NUM_COLUMNS &&
              Array.from({ length: NUM_COLUMNS - row.length }).map((_, i) => (
                <View key={`fav-empty-${i}`} className="flex-1 items-center gap-2 px-1" />
              ))}
          </View>
        ))}
      </View>
