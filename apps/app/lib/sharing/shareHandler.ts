...
export async function handleSharedUrl(url: string, metaTitle?: string): Promise<{ saved: boolean; title: string }> {
  let title = metaTitle || "";
  if (!title) {
    ...
    if (existing.data && existing.data.length > 0) {
      if (!existing.data[0].isWish) {
        await memoService.updateMemo({
          ...
          request: { url, title: existing.data[0].title, memo: existing.data[0].memo ?? "", favIconUrl: existing.data[0].favIconUrl ?? favIconUrl, isWish: true },
        });
      }
    } else {
      await memoService.insertMemo({ url, title, memo: "", favIconUrl, isWish: true });
    }
  } else {
