...
export default async function Hero({ lng }: HeroProps) {
	...
	return (
		<section className="relative py-20 lg:py-32 overflow-hidden">
			<div className="absolute inset-0 bg-gradient-to-br from-emerald-50 via-background to-teal-50 dark:from-background dark:via-background dark:to-muted" />
				...
				<div className="text-center">
					<div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 mb-6">
						...
						<span className="text-sm font-medium">
							{t("useCases.techArticle.hero.badge")}
						</span>
					</div>

					<h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground mb-6">
						{t("useCases.techArticle.hero.title")}
						<span className="block text-emerald-500 mt-2">
							{t("useCases.techArticle.hero.titleHighlight")}
						</span>
					</h1>

					<p className="text-lg sm:text-xl text-muted-foreground max-w-2xl mx-auto mb-8">
						{t("useCases.techArticle.hero.description")}
					</p>
					...
					<div className="flex flex-col sm:flex-row items-center justify-center gap-4">
						...
						>
							...
							{t("useCases.techArticle.hero.cta")}
						</Link>
						<Link
							href={`/${lng}/introduce`}
						...
						>
							{t("useCases.techArticle.hero.learnMore")}
						</Link>
					</div>
				</div>
			</div>
