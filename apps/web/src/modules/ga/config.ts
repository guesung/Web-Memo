 ...
 * 데리고 들어와 있는데, 이 도메인은 레포 어디에도 나오지 않습니다. 운영 도메인이
 * `www.webmemo.site`에서 `www.webmemo.xyz`로 옮겨 가면서 남은 옛 도메인이며, 지금은
 * 레지스트리가 DNS를 내려(clientHold) 접속되지 않고 Vercel 프로젝트에서도 뗐습니다.
 * 옛 주소로 들어온 기록만 GA에 남아 있으므로 지금의 운영 지표에는 넣지 않습니다.
 *
