...
**이 선택의 프라이버시 함의를 분명히 해둔다.** 매 페이지 조회는 로그인한 사용자가 방문하는
**모든 사이트의 URL**을 매번 서버로 보낸다는 뜻이다. `normalizeUrl`
(`packages/shared/src/utils/Url.ts:20`)은 해시만 버리고 쿼리스트링(`urlObj.search`)은
그대로 남기므로, origin과 path뿐 아니라 쿼리스트링까지 Supabase 쿼리 파라미터로 나간다.
쿼리스트링에는 검색어, 이메일, 비밀번호 재설정 토큰처럼 민감한 값이 실리는 사이트가 흔하다.
이전에는 URL이 서버로 가는 시점이 (i) 사이드패널을 연 상태의 탐색, (ii) 메모 생성 액션뿐이었다.
이 기능이 들어오면 **사용자 액션 없이, 로그인 상태에서 방문하는 모든 사이트에서, 항상**
URL이 나간다. 이는 매 페이지 조회를 택한 데 따른 직접적인 결과이며, URL 목록 캐시를
도입하면(하이라이트가 있는 URL만 미리 아는 구조가 되므로) 이 노출 범위는 크게 줄어든다.
