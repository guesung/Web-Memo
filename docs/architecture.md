...
| 배포 | 웹 → Vercel(`www.webmemo.xyz`) · 확장 → 크롬 웹스토어 · 앱 → App Store. 모두 `.github/workflows/`의 `cd-*.yml`이 담당하며 릴리스는 `release.yml`·`versions.yml`입니다.<br>`daily-ga-report.yml`은 매일 07:00 KST에 GA4 지표를 슬랙 전용 채널로 보냅니다.<br>레포 유지보수 자동화는 `cleanup-unused.yml` — 주 1회(토 10:00 KST) knip으로 미사용 파일을 찾아 정리 PR을 엽니다(설정은 루트 `knip.jsonc`) |
