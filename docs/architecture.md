...
| 배포 | 웹 → Vercel(`www.webmemo.xyz`) · 확장 → 크롬 웹스토어 · 앱 → App Store. 모두 `.github/workflows/`의 `cd-*.yml`이 담당하며 릴리스는 `release.yml`·`versions.yml`입니다.<br>`daily-ga-report.yml`은 매일 07:00 KST에 GA4 지표를 슬랙 전용 채널로 보냅니다.<br>`supabase-audit.yml`은 매일 08:00 KST와 PR에서 운영 Supabase를 저장소 선언과 대조합니다. 정기 실행은 이상을 Slack으로 알리고 PR은 오류가 있으면 실패합니다.<br>`seo-monitor.yml`은 매일 09:17 KST에 robots.txt·sitemap.xml·PC/모바일 SSR 메타·hreflang·JSON-LD·OG 이미지·URL 정규화를 검사합니다. 이전 master 실행과 회귀를 비교하고, `GSC_SERVICE_ACCOUNT_JSON`이 있으면 색인 상태와 월요일 주간 검색 성과도 조회해 Actions 요약·아티팩트로 남깁니다.<br>레포 유지보수 자동화는 `cleanup-unused.yml` — 주 1회(토 10:00 KST) knip으로 미사용 파일을 찾아 정리 PR을 엽니다(설정은 루트 `knip.jsonc`)<br>`refactor-audit.yml`은 같은 날 10:17 KST에 설계·구조·퀄리티 관점의 리팩토링 후보를 점검해 노션 작업 카드로 만들고 슬랙으로 알립니다(Claude Code 구독 토큰 사용, 상세는 [환경 변수 문서](environment-variables.md)) |
...
SEO 탭 5개의 컬럼명은 한글로 표시합니다. 기존 영문 헤더가 정의된 컬럼 순서와
...
| 항목 | 내용 |
| --- | --- |
| 프레임워크 | 웹 → **Next.js 16.3.5 App Router** (`apps/web`), React 19.1.0. `dev`·`build`·`preview` 모두 Next 16 기본값인 Turbopack을 씁니다(`--webpack` 없음). 번들 분석은 `pnpm -F @web-memo/web build:analyze`(`next experimental-analyze`, Turbopack 전용)로 하고, Sentry 소스맵은 빌드 뒤 업로드 훅(`@sentry/nextjs` 10.13+·Next 15.4.1+)으로 올라갑니다<br>확장 → **Vite 5.3.3 + Manifest V3** (`apps/chrome-extension` + `pages/*`), HMR은 `packages/hmr`<br>앱 → React Native / Expo (`apps/app`) |
| 라우팅 | `apps/web/src/app/[lng]/` 아래 `(no-auth)`(소개·기능·유스케이스·로그인·개인정보)와 `(auth)/(sidebar)`(메모·하이라이트·설정·휴지통) 두 그룹. 경로 문자열은 **`packages/shared/src/constants/Path.ts`의 `PATHS`**를 씁니다 — 하드코딩하지 않습니다.<br>페이지 경로는 손으로 등록하지 않습니다. `apps/web/scripts/generatePagePaths.mjs`가 `[lng]` 아래 `page.tsx`에서 `generatedPagePaths.ts`를 만들고(웹 `dev`·`build` 전에 실행, 생성 파일은 커밋), `PATHS`는 이것과 `MANUAL_PATHS`를 합칩니다. 키는 `[lng]`·라우트 그룹을 뺀 세그먼트를 camelCase로 이은 것이고(`/memos/wish` → `memosWish`, `/` → `root`), 동적·병렬·인터셉트 라우트나 키 충돌이 있으면 생성기가 멈춥니다. 그런 경로와 외부 도메인·route handler는 `MANUAL_PATHS`에 직접 적습니다. dev를 켠 채 페이지를 추가했으면 `pnpm -F @web-memo/web generate:paths`를 실행합니다. CI는 생성 결과가 커밋본과 다르면 실패합니다 |
...
| 운영 감사 | 기대 상태는 `.github/supabase-audit-manifest.json`, 감사 구현은 `.github/scripts/audit-supabase.mjs`가 원천입니다. 프로젝트·DB 서비스 건강 상태와 PostgreSQL 버전, migration history, 필수 앱 스키마 객체, Cron·trigger·Database Webhook·Vault key 이름, 전체 Edge Function의 이름·배포 버전·상태와 최근 24시간 오류, secret 이름을 읽기 전용으로 수집합니다. 직접 실행한 SQL은 migration history에 없을 수 있으므로 history 차이는 경고로 표시하고, 필수 테이블·컬럼·DB 함수 시그니처 누락을 오류로 처리합니다. 실행 결과에는 이상뿐 아니라 현재 운영 인벤토리를 항상 표시합니다.<br>정확한 Edge Runtime/Deno 버전은 공식 API에서 제공하지 않아 관측 불가 경고로 남깁니다. Management API의 읽기 전용 SQL이 관리 스키마를 읽지 못하면 리소스 누락과 구분하되, 필수 상태를 검증할 수 없으므로 감사를 실패시킵니다. 값·URL·명령·함수 본문·원문 로그는 수집하지 않습니다. 상세 범위와 운영 방법은 [`supabase-operations-audit.md`](supabase-operations-audit.md)를 따릅니다 |
...
| 시드 데이터 | 별도 시드 명령이 없습니다. 파일명으로 두 갈래를 가릅니다.<br>· `*.test.ts` — `e2e/tests/lib/mocks/supabaseRoutes.ts`가 네트워크를 가로채 고정 데이터를 줍니다. **기본적으로 여기에 얹으세요**<br>· `*.real.test.ts` — 로그인 외에 **실제 프로덕션 Supabase 데이터를 읽거나 씁니다.** 데이터를 남기므로 만든 것은 반드시 정리합니다. SSR이 서버에서 읽는 화면(휴지통 첫 렌더·사이드바 카테고리)은 Playwright로 가로챌 수 없어 이쪽으로 갑니다 |
| 뷰포트 | **`web`**(Desktop Chrome)이 기본값입니다. 확장 사이드 패널만 폭이 좁은(≈400px) 단일 컬럼이라 별도로 봅니다 |
| 알려진 함정 | 사이드 패널은 **활성 탭의 URL을 따라갑니다.** 탭이 바뀌거나 이동하면 백그라운드가 `UPDATE_SIDE_PANEL`을 보내고(`apps/chrome-extension/lib/background/index.ts`의 `tabs.onActivated`·`onUpdated`), 패널이 탭과 메모를 다시 조회합니다(`pages/side-panel/src/components/SidePanelContent.tsx`). 그래서 패널을 연 채 `goto`하면 새 URL의 메모를 보여 줍니다.<br>Radix Dialog와 드롭다운이 겹쳐 닫히면 `body`에 `pointer-events: none`이 남아 화면이 먹통이 됩니다. 렌더 문제로 오진하지 마세요 |
