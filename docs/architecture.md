...
| 운영 감사 | 기대 상태는 `.github/supabase-audit-manifest.json`, 감사 구현은 `.github/scripts/audit-supabase.mjs`가 원천입니다. 프로젝트·서비스 건강 상태, PostgreSQL major, migration, Edge Function 배포와 최근 24시간 오류, secret 이름, Cron, trigger, Database Webhook, Vault key 이름을 읽기 전용으로 검사합니다.<br>정확한 Edge Runtime/Deno 버전은 공식 API에서 제공하지 않아 관측 불가 경고로 남깁니다. Management API의 Beta 읽기 전용 SQL이 관리 스키마를 읽지 못하면 리소스 누락과 구분하되, 필수 상태를 검증할 수 없으므로 감사를 실패시킵니다. 값·URL·명령·원문 로그는 수집하지 않습니다 |
