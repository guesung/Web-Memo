...
| 배포 | 웹 → Vercel(`www.webmemo.xyz`) · 확장 → 크롬 웹스토어 · 앱 → App Store. 모두 `.github/workflows/`의 `cd-*.yml`이 담당하며 릴리스는 `release.yml`·`versions.yml`입니다.<br>`daily-ga-report.yml`은 매일 07:00 KST에 GA4 지표를 슬랙 전용 채널로 보냅니다.<br>`supabase-audit.yml`은 매일 08:00 KST와 PR에서 운영 Supabase를 저장소 선언과 대조합니다. 정기 실행은 이상을 Slack으로 알리고 PR은 오류가 있으면 실패합니다.<br>레포 유지보수 자동화는 `cleanup-unused.yml` — 월 1회 knip으로 미사용 파일을 찾아 정리 PR을 엽니다(설정은 루트 `knip.jsonc`) |
...
| 운영 감사 | 기대 상태는 `.github/supabase-audit-manifest.json`, 감사 구현은 `.github/scripts/audit-supabase.mjs`가 원천입니다. 프로젝트·DB 서비스 건강 상태와 PostgreSQL 버전, 전체 migration·Cron·trigger·Database Webhook·Vault key 이름, 전체 Edge Function의 이름·배포 버전·상태와 최근 24시간 오류, secret 이름을 읽기 전용으로 수집합니다. 실행 결과에는 이상뿐 아니라 현재 운영 인벤토리를 항상 표시합니다.<br>정확한 Edge Runtime/Deno 버전은 공식 API에서 제공하지 않아 관측 불가 경고로 남깁니다. Management API의 Beta 읽기 전용 SQL이 관리 스키마를 읽지 못하면 리소스 누락과 구분하되, 필수 상태를 검증할 수 없으므로 감사를 실패시킵니다. 값·URL·명령·원문 로그는 수집하지 않습니다. 상세 범위와 운영 방법은 [`supabase-operations-audit.md`](supabase-operations-audit.md)를 따릅니다 |
