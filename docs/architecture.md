...
| 배포 | 웹 → Vercel(`www.webmemo.xyz`) · 확장 → 크롬 웹스토어 · 앱 → App Store. 모두 `.github/workflows/`의 `cd-*.yml`이 담당하며 릴리스는 `release.yml`·`versions.yml`.<br>**레포에서 유일하게 `schedule` 크론으로 도는 워크플로는 `daily-ga-report.yml`입니다** (매일 07:00 KST, GA4 지표를 슬랙 전용 채널로). 크론이 없는 줄 알고 또 만들지 않도록 적어 둡니다 |
