...
| 배포 | 웹 → Vercel(`www.webmemo.xyz`) · 확장 → 크롬 웹스토어 · 앱 → App Store. 모두 `.github/workflows/`의 `cd-*.yml`이 담당하며 릴리스는 `release.yml`·`versions.yml`.<br>**`schedule` 크론으로 도는 워크플로는 둘입니다** — `daily-ga-report.yml`(매일 07:00 KST, GA4 지표를 슬랙 전용 채널로)과 `cleanup-unused.yml`(매월 1일, knip으로 미사용 파일을 찾아 정리 PR 생성 · 설정은 루트 `knip.jsonc`). 크론이 없는 줄 알고 또 만들지 않도록 적어 둡니다 |
