...
웹 배포(약 1분)를 막지 않습니다. 대기하는 것은 같은 타깃의 중복 제출뿐입니다.
워크플로 하나를 한 그룹으로 묶으면 서로 다른 타깃끼리도 무한정 줄을 서게 됩니다
(`cancel-in-progress: false`라 취소가 아니라 대기입니다).
...
| `changes` | 결과 | 댓글 |
| --- | --- | --- |
| true | success | ✅ 성공 + 배포 · 다른 버전 · 워크플로 버튼 |
...
- **과거 커밋·태그를 고른 상태에서는 버전을 못 바꿉니다.** 버전 커밋은 `master`
