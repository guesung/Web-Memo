...
`daily-article-reminder`는 운영에 배포되어 있어 기준선에 포함하지만 현재 저장소에는 함수 소스가 없습니다. `get-categories-with-count`는 운영에 남아 있으나 저장소 호출처와 외부 트리거가 없어 제거 대상으로 분류했고 기준선에서는 제외했습니다. Auth 보존 Cron과 Billing Cron도 운영 기준선에는 포함하지만 현재 브랜치에는 생성 SQL이 없습니다. 감사는 기준선 리소스의 삭제·비활성·버전 변화를 감지할 수 있지만 소스 재현성까지 보장하지는 않습니다.
