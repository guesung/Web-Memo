...
type MutationData = Awaited<ReturnType<FollowService["unfollowUser"]>>;
type MutationError = Error;

export default function useUnfollowUserMutation() {
	const queryClient = useQueryClient();
	const { data: supabaseClient } = useSupabaseClientQuery();

	return useMutation<MutationData, MutationError, MutationVariables>({
		mutationFn: ({ followerId, followingId }) =>
			new FollowService(supabaseClient).unfollowUser(followerId, followingId),
