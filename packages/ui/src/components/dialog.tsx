...
>(({ className, ...props }, ref) => (
	<DialogPrimitive.Overlay
		ref={ref}
		className={cn(
			"data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=open]:duration-base data-[state=closed]:duration-fast fixed inset-0 z-50 bg-black/40",
			className,
		)}
		{...props}
	/>
		...
		<DialogPortal>
			...
			<DialogPrimitive.Content
				...
				onEscapeKeyDown={(event) => {
					event.preventDefault();
					const hadFloating = hadFloatingLayerRef.current;
					hadFloatingLayerRef.current = false;
					if (hadFloating) return;
					const hasFloatingNow = !!document.querySelector(
						"[data-radix-popper-content-wrapper]",
					);
					if (hasFloatingNow) return;
