  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="mx-auto max-w-4xl space-y-12">
      {UPDATE_LIST.map(({ version, date }) => (
        <motion.div key={version} variants={item} className="border-border border-b pb-8">
          <div className="mb-4 flex items-baseline gap-4">
            <h2 className="text-2xl font-semibold">{version}</h2>
            <span className="text-muted-foreground">{date}</span>
          </div>
          <h3 className="mb-4 text-xl">{t(`updates.versions.${version}.title`)}</h3>
          <ul className="list-inside list-disc space-y-2">
            {(t(`updates.versions.${version}.content`, { returnObjects: true }) as string[]).map(
